...
export default function PopUpNote({ showPopUp, setShowPopUp, selectedNote, hyphenText }) {
  ...
  const handleClickEditButton = (e) => {
    ...
    const addNewTagsToText = (text, tags) => {
    ...
    const tagName = e.target.classList.value.split('')[0];
    ...
    const updatedTags = toggleTag(pairedTags, selection);
    const textWithoutTags = deleteTagsFromText(textContent);
    const arrTags = deconstructTags(updatedTags);
    
    let newTextWithTags = addNewTagsToText(textWithoutTags, arrTags);

    textAreaContent.value = newTextWithTags;
    textAreaContent.selectionStart = selection.start;
    textAreaContent.selectionEnd = selection.end;

    setCharacterCount(newTextWithTags.length);
    selectedNote.content = newTextWithTags;
  }
